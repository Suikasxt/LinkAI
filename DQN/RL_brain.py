...
from maze_env import Width
INF = 1e9
name = '/1_' + str(Width)
save_path = './ckpt' + name + name + '.ckpt'
	...
	def _build_net(self):
		...
		with tf.variable_scope('eval_net'):
			...
			c_names, n_l1, n_l2, w_initializer, b_initializer = \
				['eval_net_params', tf.GraphKeys.GLOBAL_VARIABLES], 1000, 400, \
	...
	def choose_action(self, observation):
		...
		if np.random.uniform() < self.epsilon:
			...
			for i in range(self.n_actions):
				if (self.maze.judgeValidity(i) == False):
					actions_value[0][i] -= INF
