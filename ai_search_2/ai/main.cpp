...
double search(const Map& mainGame, int deepth, int& res, int player, double cutValue, int*operaList = NULL) {
	int playerNumber = mainGame.getPlayerNumber();
	std::priority_queue<std::pair<double, int> > opaValue;
	if (player == 0) {
		...
		for (int i = 0; i < 20 && !opaValue.empty(); i++) {
			operaList[0] = opaValue.top().second;
			int opa;
			double score = search(mainGame, deepth, opa, 1, maxScore, operaList) * (1 - WeightNow) + opaValue.top().first * WeightNow;
			if (score >= cutValue) {
				delete[] operaList;
				return score;
			}
		...
		for (int i = 0; i < 20 && !opaValue.empty(); i++) {
			Map tmpGame(mainGame);
			operaList[1] = opaValue.top().second;
			for (int k = 2; k < playerNumber; k++) {
				operaList[k] = -1;
			}
			tmpGame.doOpeartor(operaList);
			int opa;
			double score = search(tmpGame, deepth - 1, opa, 0, minScore) * (1 - WeightNow) - opaValue.top().first * WeightNow;
